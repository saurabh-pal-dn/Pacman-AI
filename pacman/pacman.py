...
class GameState:
  ...
  def generateSuccessor( self, agentIndex, action):
    ...
    for i in range(1,state.getNumAgents()):
      distance=(state.getGhostPosition(i)[0]-state.getPacmanPosition()[0])**2+(state.getGhostPosition(i)[1]-state.getPacmanPosition()[1])**2
      if(distance<4):
        state.data.scoreChange -=distance*10
    """    
